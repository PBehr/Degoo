...
def encrypt_or_decrypt_file(filename, key, outFile,mode="encrypt"):

  associated_data = b'' 
  #if not FLAGS.associated_data else bytes(
  ...
  with open(key, 'rt') as keyset_file:
    try:
...
def main(argv):
  ...
  if FLAGS.mode == 'generate':
    ...
    try:
      key_template = streaming_aead.streaming_aead_key_templates.AES128_GCM_HKDF_1MB 
